...
//🪐 Made with ❤ by the devs at Celestials Stellar Club 🪐
...
contract CelestialsContract is ERC721Enumerable, Ownable {
  using Strings for uint256;

  string baseURI;
  string public baseExtension = ".json";
  uint256 public cost = 0.25 ether;
  uint256 public maxSupply = 4962;
  uint256 public maxMintAmount = 3;
  uint256 public stopSupplyMintAt = 2230;
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    if (msg.sender != owner()) {
      require(msg.value >= cost * _mintAmount, "Cannot afford to forge exoplanet.");
    }

    addressesMinted[msg.sender] = addressesMinted[msg.sender] + _mintAmount;
  ...
  function setStopSupplyMintingAt(uint256 _supplyStop) public onlyOwner {
    stopSupplyMintAt = _supplyStop;
  }
